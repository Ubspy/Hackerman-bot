 ...
 * @author Ubspy
 * @desc
   Manages adding songs to the queue, as well as starting the stream dispatcher for the music bot
 *
...
exports.run = (message, args, logger) => {
	...
	if(message.member.roles.cache.find(role => role.name.toLowerCase() === config.djRole.toLowerCase()))
	{
		...
		else // If all the requirements are met
		{
			...
			if(userChannel)
			{
				fetchVideoID(options, 0, message)
					.then(videoID => {
...
fetchVideoID = (options, index, message) => {
	return new Promise((resolve, reject) => {
		...
		search.cse.list(options)
			.then(res => {
				// Gets url from search to play
				var videoUrl = res.data.items[index].formattedUrl;

				try // Here we're going to TRY and get the video id, if it fails the video is gonna need to be fixed
				{
				...
				catch(error)
				{
					...
					fetchVideoID(options, index+1, message)
						.then(videoID => {
							resolve(videoID);
						});
				}
